...
impl RadixTarget {
    ...
    pub fn delete(&mut self, value: &str) -> bool {
        ...
        *self.cached_hash.lock().unwrap() = None;
        ...
        if deleted && !value.parse::<IpNet>().is_ok() && !value.parse::<IpAddr>().is_ok() {
            let canonical = normalize_dns(value);
    ...
    pub fn defrag(&mut self) -> (HashSet<String>, HashSet<String>) {
        ...
        *self.cached_hash.lock().unwrap() = None;
        ...
        self.hosts.retain(|h| !normalized_cleaned.contains(h) && !cleaned.contains(h));
...
mod tests {
    ...
    fn test_dns_subdomain_matching() {
        ...
        assert_eq!(rt.get("wat.hm.api.test.www.example.com"), Some("api.test.www.example.com".to_string()));
        assert_eq!(rt.get("notfound.com"), None);
    }
    ...
    fn test_prune_dns() {
        ...
        assert_eq!(rt.get("wat.hm.api.test.www.example.com"), Some("example.com".to_string()));
    ...
    fn test_hash_same_hosts_different_order() {
        ...
        assert_eq!(hash1, hash2, "Targets with same hosts in different order should have same hash");
    ...
    fn test_hash_strict_vs_non_strict() {
        ...
        assert_ne!(hash_strict, hash_non_strict, "Strict and non-strict targets should have different hashes");
        assert_ne!(rt_strict, rt_non_strict, "Targets should not be equal since equality is now hash-based");
    }
    ...
    fn test_hash_missing_host_scenario() {
        ...
        assert_ne!(hash1_before, hash2_before, "Targets with different hosts should have different hashes");
        ...
        assert_eq!(hash1_after, hash2_after, "Targets should have same hash after adding missing host");
        assert_eq!(rt1, rt2, "Targets should be equal after adding missing host");
        
        // Verify that rt1's hash didn't change (it was cached)
        assert_eq!(hash1_before, hash1_after, "rt1 hash should remain the same (cached)");
    }
    ...
    fn test_hash_caching_and_invalidation() {
        ...
        let hash1 = rt.hash();
        let hash2 = rt.hash();
        assert_eq!(hash1, hash2, "Consecutive hash calls should return same value");
        ...
        assert_eq!(hash1, hash5, "Hash should return to original after deleting added host");
    ...
    fn test_empty_target_hash() {
        ...
        assert_eq!(hash1, hash2, "Empty non-strict targets should have same hash");
        assert_ne!(hash1, hash3, "Empty strict and non-strict targets should have different hashes");
    }
    ...
    fn test_hash_consistency_across_operations() {
        ...
        assert_eq!(final_hash, rt2.hash(), "Final hashes should be equal regardless of insertion order");
        assert_eq!(rt, rt2, "Targets should be equal with same hosts and same strict_scope");
        ...
        assert_eq!(original_hash, restored_hash, "Hash should be same after adding and removing same host");
    }
    ...
    fn test_equality_with_same_strict_scope() {
        ...
        for rt in [&mut rt1_strict, &mut rt2_strict, &mut rt1_non_strict, &mut rt2_non_strict] {
            rt.insert("example.com");
            rt.insert("192.168.1.0/24");
        }
        
        // Targets with same strict_scope should be equal
        assert_eq!(rt1_strict, rt2_strict, "Strict targets with same hosts should be equal");
        assert_eq!(rt1_non_strict, rt2_non_strict, "Non-strict targets with same hosts should be equal");
        
        // Targets with different strict_scope should not be equal
        assert_ne!(rt1_strict, rt1_non_strict, "Strict and non-strict targets should not be equal");
        assert_ne!(rt2_strict, rt2_non_strict, "Strict and non-strict targets should not be equal");
    }
    ...
    fn test_ip_normalization_single_hosts() {
        ...
        assert!(rt.hosts.contains("192.168.1.100/32"), "IPv4 address should be stored as /32");
        assert!(!rt.hosts.contains("192.168.1.100"), "IPv4 address should not be stored without /32 suffix");
        
        // Insert individual IPv6 address
        rt.insert("dead::beef");
        // Should be stored as /128 network
        assert!(rt.hosts.contains("dead::beef/128"), "IPv6 address should be stored as /128");
        assert!(!rt.hosts.contains("dead::beef"), "IPv6 address should not be stored without /128 suffix");
        
        // Insert /32 IPv4 network explicitly
        rt.insert("10.0.0.1/32");
        assert!(rt.hosts.contains("10.0.0.1/32"), "IPv4 /32 should be stored as /32");
        assert!(!rt.hosts.contains("10.0.0.1"), "IPv4 /32 should not be stored without /32 suffix");
        
        // Insert /128 IPv6 network explicitly
        rt.insert("cafe::1/128");
        assert!(rt.hosts.contains("cafe::1/128"), "IPv6 /128 should be stored as /128");
        assert!(!rt.hosts.contains("cafe::1"), "IPv6 /128 should not be stored without /128 suffix");
        
        // Insert IP networks with actual network bits (should remain as-is)
        rt.insert("10.0.0.0/8");
        rt.insert("cafe::/64");
        assert!(rt.hosts.contains("10.0.0.0/8"), "IPv4 network should remain as-is");
        assert!(rt.hosts.contains("cafe::/64"), "IPv6 network should remain as-is");
        
        // Insert DNS name (should remain as-is)
        rt.insert("example.com");
        assert!(rt.hosts.contains("example.com"), "DNS name should remain as-is");
        ...
        assert_eq!(rt.get("192.168.1.100"), rt.get("192.168.1.100/32"), "IPv4 lookups should be equivalent");
        assert_eq!(rt.get("dead::beef"), rt.get("dead::beef/128"), "IPv6 lookups should be equivalent");
        ...
        assert_eq!(rt.hosts, expected_hosts, "Hosts should contain consistent network forms");
    }
